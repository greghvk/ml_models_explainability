...
def examine_interpretation(model, X, y, importances, epsilon=5, resolution=10, count_per_step=5, proportionality_mode=0, from_zero=False, categorical_cols=[], probability_multiplier=1):
    """
    """
    ...
    if from_zero:
        intermediate_importances = create_intermediate_points(np.zeros(len(inverse_importance_shares)), inverse_importance_shares, resolution)
    else:
...
def gradual_elimination(model, X, y, importances, test_X=None, test_y=None, single_drop=False, categorical_cols=[], removal_order=[]):
    ...
    sns.barplot(x=importances_df['Feature names'], y=importances_df[1], edgecolor='k', color='aliceblue', linewidth=2, order=importances_df[0])
    plt.ylabel('Importance share')
    plt.xlabel('Dropped feature name')

    sns.set_style("white")
    ax2 = fig.add_subplot(111, sharex=ax1, frameon=False)
    sns.lineplot(x=[i for i in range(len(importances))], y=importances_df['Loss of accuracy'], marker='o', color='m', linewidth=2)
